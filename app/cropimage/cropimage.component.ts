...
export class CropimageComponent implements OnInit {
  ...
  code; point; style; drag: any = null; dPoint;
  interval; progress
  xCord: any; yCord: any;
  frameIndex: number = 0;
  ...
  framelist: any; Iframe = [];
  constructor(private router: Router, private _cropimageService: CropimageService) { }
  ngOnInit() {
    this.Overlaystyle = 'block';
    this.btnnext = 'Next';
    this.pageloadsetting();
    // 
    this.getPreviewImages(JSON.parse(sessionStorage.getItem("getindeximages")), localStorage.getItem("uploadedfile"));
  ...
  finalsubmit() {
   ...
   this.Iframe=[];
    ...
    this._cropimageService.submitBoundriesNframes(this.Iframe, this.FinalBoundriescords, localStorage.getItem("uploadedfile")).subscribe(
      framelist => {
      ...
      data => {
        if (data !== undefined || data !== null) {
          ...
          for (let i = 0; i < data.Value.Cords.length; i++) {
            this.drawline(data.Value.Cords[i].split(',')[0], data.Value.Cords[i].split(',')[1]);
          ...
          for (let i = 0; i < data.Value.DotCords.length; i++) {
            this.drawCoordinates(data.Value.DotCords[1].split(',')[0], data.Value.DotCords[1].split(',')[1]);
            var cols = {
