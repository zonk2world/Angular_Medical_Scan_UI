...
export class BoundarypreviewComponent implements OnInit {
  ...
  outpath = environment.outputfolderpath;
  firstimg: string;
  ...
  frameList: any = [];
  frame_Title_Range_Index = 1;
  frameImageIndex = 0;
  FinalBoundriescords = [];
  constructor(
    private router: Router,
    private _downloadservice: CropvideoService,
    private element: ElementRef,
    private _drawInterpolationService: DrawInterpolationService,
  ...
  finalsubmit() {
    ...
    for (let i = 0; i < this.frameList.length - 1; i++) {
      this._drawInterpolationService.connectDotsOverFrames(
        this.frameList[i].getindeximages,
        this.frameList[i + 1].getindeximages,
        "cord"
      );
    }
    for (
      let i = 1;
      i <= this.frameList[this.frameList.length - 1].getindeximages;
      i++
    ) {
  ...
  getPreviewImages() {
    this._downloadservice
      .GetImages(localStorage.getItem("uploadedfile"), "2")
      .subscribe(
        (imageslist) => {
  ...
  play() {
    ...
    var interval = setInterval(() => {
      if (this.frameImageIndex < parseInt(this.ddEnd)) {
        this.image.src =
          this.objimagesNamelist[this.frameImageIndex].fileInBytes;

        this.frameImageIndex++;
        if (this.frame_Title_Range_Index < this.ddEnd) {
          this.frame_Title_Range_Index++;
        }
        this.drawBaundries(this.frame_Title_Range_Index);
        this.drawdots(this.frame_Title_Range_Index);
  ...
  drawdots(frameImageIndex) {
    if (
      sessionStorage.getItem("points" + frameImageIndex.toString()) !== null
    ) {
      var dots = sessionStorage
        .getItem("points" + frameImageIndex.toString())
        .toString()
        .split(",");
  ...
  addFrameBoundary() {
    ...
    sessionStorage.setItem(
      "addframeImgbytes",
      this.objimagesNamelist[this.frame_Title_Range_Index - 1].fileInBytes
    );
    this.frameList = JSON.parse(sessionStorage.getItem("getindeximages"));
    var fi = {
  ...
  deleteFrameBoundary(index) {
    if (this.validateFrameBeforDelete(index)) {
      ...
      for (let i = 0; i < frameIndex.length - 1; i++) {
        this._drawInterpolationService.connectDotsOverFrames(
          frameIndex[i].getindeximages,
          frameIndex[i + 1].getindeximages,
          "cord"
        );
      }
  ...
  validateFrameBeforDelete(index) {
    this.frameList = JSON.parse(sessionStorage.getItem("getindeximages"));
    if (
      this.frameList[0].getindeximages == index ||
      this.frameList[this.frameList.length - 1].getindeximages == index
    ) {
      return false;
    }
    return true;
